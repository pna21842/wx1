...
public:
    ...
    MyTreeCtrl(wxFrame* parent) : wxTreeCtrl(parent) {
...
public:
    ...
    GLFrame3(const wxString& title) : wxFrame(nullptr, wxID_ANY, title, wxDefaultPosition, wxDefaultSize) {
        ...
        // create controls and associated event handlers
        ...
        auto colourButton = new wxButton(this, wxID_ANY, "Change Color");

        // Setup event handling for colourButton
        colourButton->Bind(wxEVT_BUTTON, [this](wxCommandEvent& event)
            {
        ...
        // tree view model
        auto treeView = new MyTreeCtrl(this);

        auto root = treeView->AddRoot("Angel Family");
         
        auto node11 = treeView->AppendItem(root, "Dad");
        auto node12 = treeView->AppendItem(root, "Harley");

        auto node21 = treeView->AppendItem(node12, "Froggy");
        //auto node22 = treeView->AppendItem(node12, "node 2-2");
        auto node23 = treeView->AppendItem(node12, "node 2-2");

        auto node13 = treeView->AppendItem(root, "Maisie");
        auto node31 = treeView->AppendItem(node13, "Tommy");

        auto node14 = treeView->AppendItem(root, "Mum");

        
        // Sort layout
        auto top_level_sizer = new wxBoxSizer(wxVERTICAL);
        auto gl_view_sizer = new wxBoxSizer(wxHORIZONTAL);
        auto button_sizer = new wxBoxSizer(wxHORIZONTAL);

        gl_view_sizer->Add(gl_canvas, 3, wxEXPAND);
        gl_view_sizer->Add(treeView, 1, wxEXPAND);

        button_sizer->Add(colourButton, 0, wxALL | wxALIGN_CENTER, FromDIP(15));
        button_sizer->AddStretchSpacer(1);

        
        top_level_sizer->Add(gl_view_sizer, 1, wxEXPAND);
        top_level_sizer->Add(button_sizer, 0, wxFIXED_MINSIZE);
